  ...
  const renderOcrItems = () => {
    ...
    itemsEl.querySelectorAll('.subtitle-item-use-btn').forEach((btn) => {
      btn.addEventListener('click', async (event) => {
        ...
        setBusy(true);
        try {
          const result = await api(`/api/assets/${asset.id}/video-ocr`, { method: 'PATCH', body: JSON.stringify({ itemId }) });
          applyAssetFromApi(result.asset);
          renderOcrItems();
        } catch (error) {
    ...
    itemsEl.querySelectorAll('.subtitle-item-remove-btn').forEach((btn) => {
      btn.addEventListener('click', async (event) => {
        ...
        setBusy(true);
        try {
          const result = await api(`/api/assets/${asset.id}/video-ocr`, { method: 'DELETE', body: JSON.stringify({ itemId }) });
          applyAssetFromApi(result.asset);
          renderOcrItems();
        } catch (error) {
  ...
  const watchJob = async (jobId) => {
    ...
    try {
      ...
      if (done.status === 'completed' && String(done.resultUrl || '').trim()) {
        const saved = await api(`/api/assets/${asset.id}/video-ocr/save`, { method: 'POST', body: JSON.stringify({ jobId }) });
  ...
  const restoreLatest = async () => {
    try {
      ...
      if (latest.status === 'completed' && String(latest.resultUrl || '').trim() && latest.source !== 'db') {
        const saved = await api(`/api/assets/${asset.id}/video-ocr/save`, { method: 'POST', body: JSON.stringify({ jobId: latest.jobId }) });
        applyAssetFromApi(saved.asset);
        renderOcrItems();
