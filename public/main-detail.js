  ...
  function createMainDetailModule(deps) {
    ...
    function detailMarkup(asset, workflow, options = {}) {
      ...
      const visibilitySection = Boolean(currentUserIsSuperAdmin?.() && asset.canManageVisibility) ? `
        <form id="assetVisibilityForm" class="inline-grid">
          ...
          <button type="submit" class="mam-action-btn mam-action-btn-visibility"><span class="mam-action-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M2 12s4-6 10-6 10 6 10 6-4 6-10 6S2 12 2 12z"/><circle cx="12" cy="12" r="3"/></svg></span><span class="mam-action-label">${escapeHtml(t('save_visibility'))}</span></button>
        </form>
      ` : '';
      ...
      const versionSection = canManageVersions ? `
        <form id="versionForm" class="inline-grid">
          ...
          <label class="version-file-picker">
            <input name="versionFile" type="file" class="version-file-input" />
            <span class="version-file-picker-button">${escapeHtml(t('choose_file'))}</span>
            <span class="version-file-picker-name" data-version-file-name>${escapeHtml(t('no_file_chosen'))}</span>
          </label>
    ...
    function scrollDetailPanelToVideoTop(root = assetDetail()) {
      ...
      const target = root.querySelector('.detail-video-fixed')
        || root.querySelector('.audio-detail-viewer')
    ...
    function seekOpenDetailMedia(assetId, startAtSeconds) {
      ...
      try {
        ...
        if (!(mediaEl instanceof HTMLAudioElement)) scrollDetailPanelToVideoTop(assetDetail());
        return true;
      } catch (_error) {
        return false;
      }
