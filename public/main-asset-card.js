    ...
    function render(asset, searchState = {}, searchHighlightClass = 'search-hit') {
      ...
      const attachmentHits = hasTextSearch ? (asset.attachmentSearchItems || []).flatMap((file) => {
        return [['Ad', file.label], ['Açıklama', file.note], ['Dosya adı', file.fileName]].flatMap(([field, value]) => {
          ...
          return [`<div class="asset-meta dc-hit-row"><button type="button" data-field-jump="1" data-id="${escapeHtml(asset.id)}" data-attachment-id="${escapeHtml(file.versionId || '')}" style="text-align:left;white-space:normal;background:transparent;color:inherit;border:0;border-radius:0;box-shadow:none;padding:0;font:inherit;"><span>Ek dosya · ${escapeHtml(file.label || file.fileName || '')} · ${field}: </span>${highlighted}</button></div>`];
        });
      }).join('') : '';
