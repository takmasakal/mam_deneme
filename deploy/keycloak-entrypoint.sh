...
if [ -f "$template_path" ]; then
  ...
  mam_superadmin_password="$(escape_sed_file /run/secrets/mam_superadmin_password)"
  ...
  mam_superadmin_user_escaped="$(printf '%s' "${MAM_SUPERADMIN_USER:-mamsup}" | sed -e 's/[\/&]/\\&/g')"
  ...
  sed \
    ...
    -e "s|__MAM_SUPERADMIN_USER__|${mam_superadmin_user_escaped}|g" \
    -e "s|__MAM_SUPERADMIN_PASSWORD__|${mam_superadmin_password}|g" \
